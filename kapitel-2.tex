...
days before the data displayed in \fig{TSarkona}c were obtained), during which 
...
\fig{studyarea}, yellow marks). In \fig{abslope}, the cross-slope component of 
...
at that time, displayed above in \fig{TSarkona}c. As the observed 
...
silt (\fig{tauberkarte}). In the vicinity of TW (see map in \fig{studyarea}), a 
...
prevailing sediments are coarser in the shallower parts \citep[][]{basys1}. %In 
...
Tromper Wiek region (see map in \fig{studyarea}) is depositional as well. These 
...
AB in \fig{studyarea}. \fig{abmss} shows the 
profiles of 
...
\fig{transect} shows how the water body is structured along the 
...
the density in this region) in \fig{transect} show a clear cross-slope density 
gradient below the halocline. %They are tilted towards the bottom, consistent 
...
currents remained unsteady with a mean easterly component (\fig{tromperwiek}c).
...
below the halocline (see \fig{TSarkona}). This indicates that upwelling 
...
contamination of the sediment with mercury. As visible in \fig{hg}, the dumping 
...
concentrations in the fine fraction (\fig{hg}b) southeast of the Arkona Basin 
...
corresponds to medium to coarse silt. As visible in \fig{tauberkarte}, this is 
...
(\fig{tauberkarte}). Nevertheless, it can only be speculated whether 
