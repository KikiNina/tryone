...
longitudinal direction \citep[][]{lass2005}. The flow patterns in this area 
...
flows into the Arkona Basin. Coriolis effects form a 
topographically trapped dense rim current which rotates cyclonically around the 
basin, following the slope. As inflow events are driven by larger scale wind 
forcing, this process is not steady, and the dense bottom current can disappear 
completely. Parts of the saline bottom water are transferred from the cyclonic 
bottom current to the saline bottom water pool. ADD INFOS ABOUT the interplay 
between sporadic smaller inflow events and salt water drainage toward the 
Bornholm Basin (drainage time scale is of the order of a month) determine the 
position of the halocline FROM THE TWO PAPERS LARS SUGGESTED. The barotropic 
